  ...
  async run(msg, args, client) {
    if (args.length < 3) {
      const embed = new Discord.MessageEmbed()
        ...
        .setDescription(`Hey! The command argument syntax is a bit wrong. Here is the correct argument syntax for this command \`-config <crypto-name> <network-name>\`.\nFor example: \`-config matic mumbai\``)
      ...
      if (!cryptoArg) {
        ...
        if (!crypto.includes(cryptoArg)) {
          const embed = new Discord.MessageEmbed()
            .setColor(theme["error"])
            .setDescription(`We currently don't support \`${cryptoArg}\` cryptocurrency`)
          msg.reply({ embeds: [embed] })
        }
        else {
          if (networkArg !== network.get(cryptoArg)) {
            ...
            const embed = new Discord.MessageEmbed()
              ...
              .setDescription(`Successfully configured \`${cryptoArg}\` as the cryptocurrency and \`${networkArg}\` as the network.`)
            msg.reply({ embeds: [embed] })
          }
        }
      }
